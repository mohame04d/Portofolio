...
const TimelineItem = ({ item, index }) => {
  ...
  return (
    <div className={`mb-12 flex flex-col md:flex-row justify-between items-center w-full ${isLeft ? 'md:flex-row-reverse' : ''}`}>
      ...
      <div className="z-20 flex items-center bg-slate-800 shadow-xl shadow-sky-500/10 w-12 h-12 rounded-full border-4 border-slate-900 absolute left-1/2 -translate-x-1/2">
        <div className="w-full flex justify-center text-sky-400">
      ...
      >
        <span className="flex items-center gap-2 text-sky-400 font-medium mb-2 text-sm">
          <Calendar size={16} />
          {item.date}
        </span>
        <h3 className="text-xl font-bold text-slate-100">{item.title}</h3>
        <h4 className="text-lg font-medium text-slate-300 mb-3">{item.organization}</h4>
...
const Timeline = () => {
  ...
  return (
    <section id="experience" className="py-20 border-t border-slate-800 relative">
      <div className="text-center mb-16">
        <h2 className="text-3xl md:text-5xl font-bold mb-4">{t('timeline.title1')} <span className="text-gradient">{t('timeline.title2')}</span></h2>
      </div>
